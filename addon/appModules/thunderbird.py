...
class AppModule(thunderbird.AppModule):
	...
	def chooseNVDAObjectOverlayClasses(self, obj, clsList):
		...
		# ligne ci-dessous déplacée dans gainfocus
	...
	def event_gainFocus (self,obj,nextHandler):
		role = obj.role
		if role in (controlTypes.Role.LISTITEM, controlTypes.Role.TREEVIEWITEM) :   
			if utils.hasID(obj, "threadTree-row") :
				# for test> utils.listColumnNames(obj)
				sharedVars.curTTRow = obj.name
				try : 
					if not sharedVars.TTnoClean :obj.name = self.buildColumnNames(obj)
				except : pass
		elif  sharedVars.msgOpened and role == controlTypes.Role.DOCUMENT : # separate message window
			sharedVars.msgOpened = False
			return wx.CallLater(100, sharedVars.oQuoteNav.readMail, obj, False)
	...
	def buildColumnID(self, oTT):
		try :
			...
			# for e in self.columnID  :
				# sharedVars.tlog("header left:{}, ID:{}, name:{}".format( str(e[0]), e[1], e[2]))
		finally :
	...
	def buildColumnNames(self, oRow) :
		...
		# sharedVars.tlog("Option junkStatusCol:" + str(junkStatusCol))
		# playSound_unread = False #options.as_bool ("playsound_unread")
		# sharedVars.tlog("Original rowName:" + sharedVars.curTTRow)
	...
	def script_sharedEscape(self, gesture) :
		...
		if ID.startswith("threadTree-row") :
			if hasFilter(o, ID) :
				wx.CallAfter(speech.speakText, _("Filter removed"))
				return gesture.send()
		...
		# elif role == controlTypes.Role.EDITABLETEXT and utils.hasID(o.parent, "quickFilterBarContainer") :
			# if o.value is not None : speech.speakText(_("Keyword removed."))
			# return gesture.send()
	...
	def script_sharedAltN(self, gesture) :
		...
		# sharedVars.tlog("parentID=" + parID)
		rc = int(getLastScriptRepeatCount())
		...
		if mainKey in ("downArrow", "upArrow") : 
			...
			if ID.startswith("threadTree-row") :
				if controlTypes.State.COLLAPSED in o.states : 
					...
					return speech.speakText(_("Press right arrow and retry, please."))
				...
				return messengerWindow.folderTreeItem.menuFolders(o, (mainKey == "downArrow"))
	...
	def script_sharedAltHome(self, gesture) :
		...
		if rc == 0: # 1 press : search new update 
			...
			self.timer = wx.CallLater(d, messengerWindow.folderTreeItem.menuAccounts)
...
def debugShow(appMod, auto) :
	...
	sharedVars.tlog("sharedVars.curSubject :" + sharedVars.curSubject)
	if sharedVars.oQuoteNav :
		sharedVars.tlog("oQuoteNav.subject : " + sharedVars.oQuoteNav.subject)
